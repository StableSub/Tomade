...
- 평소에는 캐릭터가 통로를 걷습니다. 기존 세 연구원은 실제 조사 시작 이벤트를 받으면 자리로 이동해 조사하고, 완료하면 부장에게 보고하러 이동합니다. 뚱이(기술 분석)와 스폰지밥(투자 심리)은 자리 배정 전까지 고정 위치에 서서 조사합니다. 두 캐릭터를 클릭하면 해당 역할의 조사 결과를 확인할 수 있습니다. 일반 질문에서는 Worker가 조사하지 않습니다.
...
- 토마토 테마의 두 방 사무실과 정원이 화면 크기에 맞춰 표시됩니다. 모바일과 세로 창에서도 드래그 없이 부장·다섯 연구원·커비를 찾을 수 있습니다. 이름은 캐릭터 위를 따라다니며, 시스템의 동작 줄이기 설정에서는 산책을 멈추고 조사 상태를 즉시 표시합니다.
