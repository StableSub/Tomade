...
- `frontend/index.html`·`styles.css`가 사무실과 대화·조사 노트·대화 기록 창을 구성한다. `assets/research-office.png`는 토마토 로고·러그와 네 작업 공간을 중심으로 소품을 줄인 승인 배경이다. 캐릭터와 버튼은 배경과 별도로 표시한다.
- `app.ts`의 `setOfficeMode`는 동일한 사무실 DOM을 전체 화면과 오른쪽 미니맵 사이에서 350ms 동안 이동·축소한다. 데스크톱은 대화 64%·사무실 36%, 모바일은 전체 폭 대화와 상단 작은 미니맵이다. 미니맵에서 캐릭터는 inert 상태가 되고 전체 이미지가 사무실 복귀 버튼이 된다. 작성 중인 질문·캐릭터 상태는 전환 시 유지하며 모션 감소 설정은 전환 애니메이션을 생략한다.
...
- `office-scene.ts`의 `OfficeScene`이 4개 도트 캐릭터, 통로 경로, 이동·착석·보고 애니메이션을 관리한다. `assets/office-characters.png`는 승인된 시안 인물을 기준으로 만든 4×4 스프라이트 시트다. 행은 부장·비즈니스·매크로·이벤트, 열은 정면·후면·오른쪽 걷기·착석이며 왼쪽 걷기는 좌우 반전한다. 로딩 때 한 번 배경색을 투명 처리하고 각 자세를 128×160 Canvas에 비율을 유지해 표시한다. 부장 초상화도 같은 이미지의 정면 얼굴을 사용한다. `app.ts`는 API·대화 저장·SSE 상태를 관리하며 캐릭터에 상태만 전달한다. 이동은 시각적 표현으로 모델·도구의 실행 시점을 제어하지 않는다.
- 대기 중에는 산책한다. `assets/office-walk-{front,back,right}.png`의 방향별 4단계 전신 자세를 실제 이동 거리에 맞춰 재생하고 왼쪽은 오른쪽 그림을 반전한다. 출발·도착 시 가감속하며 정지하면 기본 자세로 돌아간다. 계획의 `tasks[].agent`로 조사 인원을 정하고 Worker의 `node.started`에 자리 이동·착석, `node.completed`에 보고 이동을 연결한다. 선택되지 않은 Worker는 계속 산책한다. 진행률은 선택된 Worker의 완료 수/선택 수다.
...
- 사무실 전체 화면에서 Worker 클릭 시 이번 실행의 보고서·스트리밍 문장을 노트 창에 표시한다. 완료 출력이 중간 Buffer를 대체한다. 대화 복원 시 Worker 보고서는 비우고 저장된 사용자 질문·최종 답변을 복원한다. pending 대화는 입력을 막고 기록 창을 다시 열 때 서버 상태를 재조회한다.
- 긴 답변·노트는 창 안에서 스크롤한다. 사무실은 가로·세로 가용 공간 중 작은 쪽에 맞춰 전체를 축소하므로 수평·수직 이동이 필요 없다. 키보드 입력·캐릭터 선택과 모션 감소 설정을 지원하고 숨겨진 탭에서는 애니메이션을 멈춘다.
- `frontend/tests/office-ui.mjs`는 모든 API를 Mock으로 가로채 이동·착석·완료·실패·대화 복원·모바일을 검증한다. 실제 모델·외부 금융 API 성능 검증과는 별개다.
