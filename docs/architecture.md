...
- `frontend/index.html`·`styles.css`가 사무실과 대화·조사 노트·대화 기록 창을 구성한다. `assets/research-office.png`는 패트와 매트 집 분위기의 16:10 승인 배경이다. 왼쪽 나무 짜임 바닥은 직원 사무실, 오른쪽 체크 타일 바닥은 부장실이며 가운데 문으로 연결된다. 캐릭터와 대화하기 버튼은 배경과 별도로 표시한다. 가구 앞면은 같은 배경의 일부를 겹쳐 착석 캐릭터의 다리가 의자 뒤에 보이도록 한다.
