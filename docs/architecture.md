...
- `app.ts`의 `setOfficeMode`는 동일한 사무실 DOM을 전체 화면과 오른쪽 미니맵 사이에서 350ms 동안 이동·축소한다. 데스크톱은 대화 64%·사무실 36%, 모바일은 전체 폭 대화와 상단 작은 미니맵이다. 미니맵에서 커비를 포함한 일곱 캐릭터는 inert 상태가 되고 사무실 전체가 복귀 버튼이 된다. 작성 중인 질문·캐릭터 상태는 전환 시 유지하며 모션 감소 설정은 전환 애니메이션을 생략한다.
...
- 대기 중에 부장은 부장실 안에서, 기존 세 직원은 왼쪽 사무실 안에서 산책한다. `assets/office-walk-{front,back,right}.png`의 새 캐릭터별 4프레임 전신 그림을 기존 이동 거리 기반 로직으로 재생하고 왼쪽은 오른쪽 그림을 반전한다. 프레임 사이를 보간하지 않으며, 방 비율을 반영한 거리 계산으로 상하·좌우 걸음 속도를 맞춘다. 출발·도착 시 가감속하며 정지하면 기본 자세로 돌아간다. 계획의 `tasks[].agent`로 조사 인원을 정하고 Worker의 `node.started`에 자리 이동·착석, `node.completed`에 가운데 문을 통과하는 보고 이동을 연결한다. 선택되지 않은 Worker는 계속 산책한다. 진행률은 선택된 Worker의 완료 수/선택 수다.
...
뚱이(`technical`)와 스폰지밥(`sentiment`)은 직원 사무실의 고정 위치에 서서 대기하며, 조사 이벤트 동안 서류를 확인하는 자세를 표시한다. 기본 책상·좌석은 배정하지 않는다. `OfficeScene.setWorkerSeat(id, point | null)`은 추후 정해진 좌석의 발 좌표를 받아 조사 중 착석 모션을 사용하고, `null`이면 다시 기존 서 있는 위치를 사용한다. 두 역할의 클릭 영역은 기존 조사 노트 창으로 연결되어 해당 실행의 보고서·근거·한계·오류를 표시한다. 기술 지표는 상승·매수를 단정하지 않고, 투자 심리는 수집한 YouTube 댓글 표본으로 범위를 명시한다. 실제 YouTube 수집 품질은 이 UI 작업의 Mock 검증에 포함하지 않는다. 두 캐릭터는 각각 대기·서서 조사·착석 A/B의 2×2 도트 에셋을 사용한다. 실제 표시 크기에 맞춰 렌더링하고 노출된 외곽 픽셀을 보정해 분수 배율에서도 어두운 경계선을 유지한다.
