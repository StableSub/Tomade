...
- 저장된 완료 질문·답변 쌍은 상위 Agent의 단기 컨텍스트에도 사용한다. error/interrupted 응답과 현재 질문은 이전 턴 집계에서 제외한다.
...
- 상위 Agent에 세션 요약과 최근 대화를 전달하며 메모리 갱신 Tool은 제공하지 않는다. 포트폴리오 자동 진단은 별도 API로 유지한다.
