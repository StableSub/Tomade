...
- `frontend/index.html`·`styles.css`가 사무실과 게임형 대화·조사 노트·대화 기록 창을 구성한다. `assets/research-office.png`는 승인된 토마토 테마 정사각형 시안에서 사람만 제거한 배경이다. Tomade 로고·자리 명패는 이미지에 유지하며 실제 캐릭터·버튼·대화창은 별도로 표시한다.
...
- Worker 클릭 시 이번 실행의 보고서·스트리밍 문장을 노트 창에 표시한다. 완료 출력이 중간 Buffer를 대체한다. 대화 복원 시 Worker 보고서는 비우고 저장된 최종 답변만 복원한다. pending 대화는 입력을 막고 기록 창을 다시 열 때 서버 상태를 재조회한다.
...
├── index.html / styles.css # 사무실·게임형 대화·노트
